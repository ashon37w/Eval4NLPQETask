    ...
    def direct_assessment_mt_block(
        ...
        return "\n".join(
            [
                ...
                "As reference, there are four other models' scores provided.",
...
if __name__ == "__main__":
    folder_path='/H1/SharedTask2023/baselines/test_results/op_mulit_oop1tooop10_chatop2'
    ...
    index_list=[ [] for _ in range(len(inputs))]
    for i in range(len(inputs)):
        ...
        differences=[]
        for j in range(10):
            differences.append( (normalized_scores[j][i]-mean,j) )
    ...
    for (src, hyp) in tqdm(inputs):
            ...
            target_lang="German",
            source_lang="English",
            as1=normalized_scores[index_list[cnt][0]][cnt],
            as2=normalized_scores[index_list[cnt][1]][cnt],
            as3=normalized_scores[index_list[cnt][2]][cnt],
            as4=normalized_scores[index_list[cnt][3]][cnt]
            )
            scores.append(score)
            prompts.append(_)
            print(score)
    ...
    with open(folder_path+'/en_de.description', 'w') as f:
        sentence='This submission uses the OpenOrca-Platypus2-13B model, incorporating the better scores from previous submissions'
        f.write(sentence+"\n")
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    get_zip('/H1/SharedTask2023/baselines/test_zips/op_mulit_oop1tooop10_chatop2.zip',lis_nam)
