...
class ForcedPrefixLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # print(self.cnt,now_idx,now_prob[now_idx].cpu().item())
...
def generate_completions(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, generate_name="",in_tag_cut=False,**generation_kwargs):
    ...
    for i in range(0, len(prompts)):
        ...
        tokenized_prompts = tokenizer([x['prompt'] for x in batch_prompts], padding="longest", return_tensors="pt", add_special_tokens=add_special_tokens)
        ...
        try:
            ...
            # for tok, score in zip(out_seq.cpu()[:len(probs)], probs):
